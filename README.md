...
仓库已包含第一阶段 Unity 内嵌远控后端源码：`crates/wincast-unity-native` 提供给 Unity 调用的 Rust native core FFI 边界，`unity/com.zoranner.wincast` 提供 Unity package runtime 源码。该后端目标是在 Unity 进程内采集最终 Game View 帧，并在 Unity 主线程消费远端输入，避免依赖 Windows 桌面抓屏和显示器电源状态。

当前源码层已具备配置解析、runtime 状态统计、帧参数校验、最终帧采集结构、native bridge 映射、本地输入事件队列、Unity UI/业务输入分发，以及 Rust native 侧 TCP 会话、协议输入入队和 H.264 编码帧发送链路；尚未完成 Unity Player 真机验证、端到端客户端验收、单实例稳定性和 UI 输入验收。详细设计见 [docs/plans/2026-06-21-Unity内嵌远控后端设计.md](docs/plans/2026-06-21-Unity内嵌远控后端设计.md)。

Host 侧可通过 `mode = "unity_embedded"` 选择 Unity 内嵌后端。该模式需要额外提供 `[unity]` 配置；Host 读取配置后只拉起一个 Unity 进程，并以 `--wincast-port <port>` 传入固定端口，随后在前台监控该 Unity 进程。Unity 进程的分辨率、FPS、码率和鉴权策略由 Unity package 或具体 Unity 项目自身配置，Host 不通过启动参数覆盖。Linux client 在该模式下应连接 Unity Player 监听的 `[unity].port`，不是连接 Host 的 `listen` 端口。
...
`unity_embedded` 当前仍复用通用配置模型，因此 `[program]`、`[video]` 和 `[capture]` 需要保留为合法配置占位；实际 Unity 进程启动使用 `[unity]`，画面参数由 Unity package 内部配置决定。
