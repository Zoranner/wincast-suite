...
配置文件默认从用户配置目录读取，日常运行不需要每次传 `--config`：Windows host 默认读取 `%APPDATA%\WinCast\wincast-host.toml`，Linux client 默认读取 `${XDG_CONFIG_HOME:-$HOME/.config}/wincast/wincast-client.toml`。`XDG_CONFIG_HOME` 必须是非空绝对路径；未设置、为空或为相对路径时回退到 `$HOME/.config`。`--config` 仅用于临时调试或一次性验证时覆盖默认路径。

设计文档见 [docs/design.md](docs/design.md)，稳定版真机烟测清单见 [docs/smoke-test.md](docs/smoke-test.md)。
