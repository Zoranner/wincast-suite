...
当前代码中的 `wincast-host` 仍是 Windows 前台可执行程序，方便先把启动、窗口定位、捕获、输入和传输链路打通。后续为了处理开机自启、登录态和锁屏状态，宿主端需要演进为 Windows Service 加用户态 Host Agent 的两层结构。
...
第一阶段只做状态检测和恢复编排，不做 Credential Provider，不采集、不保存、不转发 Windows 登录密码。若未来确实需要远程登录或远程解锁，应作为独立安全敏感模块评估，不并入当前 raw BGRA 主线。
...
- 补齐当前 raw BGRA 主线的资源释放、断线重连、窗口关闭和错误上报，先让前台 Host Agent 形态稳定。
- 抽出 Host Agent 运行核心，把网络会话、程序生命周期、捕获和输入编排从 CLI 入口中剥离出来，便于后续由 Service 拉起。
- 增加 Windows 会话状态模块，封装登录、锁屏、解锁和注销检测，并用单元测试覆盖状态转换。
- 增加 Service 与 Host Agent 的本机 IPC 协议，先传递 Agent 在线、桌面可用、会话开始、会话结束和错误状态。
- 实现 Windows Service 安装、卸载、启动、停止和开机自启，Service 只负责编排，不直接捕获和注入输入。
- 接入 Service 拉起 Host Agent 的流程，验证自动登录后可自动恢复远程服务能力。
