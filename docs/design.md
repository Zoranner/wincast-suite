...
- 宿主端可按 `program.turn_off_monitor_after_launch` 在目标程序启动成功后请求 Windows 关闭物理显示器。
...
- `program.turn_off_monitor_after_launch` 控制目标程序启动成功后是否请求 Windows 关闭宿主端物理显示器，默认关闭。
...
- Host 收到会话请求后能启动配置程序。
- `program.turn_off_monitor_after_launch = true` 时，Host 在目标程序启动成功后请求关闭宿主端物理显示器；关屏失败时清理本次启动程序并暴露错误。
...
- 关闭物理显示器依赖 Windows 显示器电源消息，鼠标、键盘、显示器或显卡驱动事件可能重新点亮屏幕，需要真机烟测确认。
