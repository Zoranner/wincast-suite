...
- 宿主端可按 `program.turn_off_monitor_after_launch` 在目标程序启动成功后处理显示器电源，默认不处理显示器电源。
...
- `program.turn_off_monitor_after_launch` 控制目标程序启动成功后的显示器电源处理，默认 `disabled`，不处理显示器电源。
- `program.turn_off_monitor_after_launch = "windows_power_message"` 表示发送 Windows 显示器电源关闭消息，可能影响 DXGI Desktop Duplication 采集。
- `program.turn_off_monitor_after_launch = "ddc_ci_power_off"` 表示通过 DDC/CI VCP 0xD6 请求显示器硬件关屏，更接近手动关闭显示器，依赖显示器、线缆和显卡驱动支持。
...
- Host 收到会话请求后能启动配置程序。
- `program.turn_off_monitor_after_launch = "disabled"` 时，Host 在目标程序启动成功后继续保持默认显示器电源状态。
- `program.turn_off_monitor_after_launch = "windows_power_message"` 时，Host 在目标程序启动成功后发送 Windows 显示器电源关闭消息；需要真机确认 DXGI Desktop Duplication 后续采集状态和失败错误暴露。
- `program.turn_off_monitor_after_launch = "ddc_ci_power_off"` 时，Host 通过 DDC/CI VCP 0xD6 请求显示器硬件关屏；需要目标 Windows 真机确认显示器支持、请求结果、DXGI 采集状态和失败错误暴露。
...
- `turn_off_monitor_after_launch = "windows_power_message"` 可能让 DXGI Desktop Duplication 采集失效，必须保留明确风险提示。
- DDC/CI 硬件关屏和调暗依赖显示器、线缆、显卡驱动与 VCP 能力支持，必须在目标 Windows 真机上烟测确认；这些模式只能作为替代方案验证，不能承诺一定解决 DXGI 采集问题。
