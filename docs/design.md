...
当前代码已用纯状态机描述这些状态转换和对客户端可见的状态/错误，但尚未接入 `WTSRegisterSessionNotification`、`WTSGetActiveConsoleSessionId`、桌面锁定事件或其他 Windows 会话 API。当前只做模型约束，不做真实状态检测和恢复编排；系统也不做 Credential Provider，不采集、不保存、不转发 Windows 登录密码。若未来确实需要远程登录或远程解锁，应作为独立安全敏感模块评估，不并入当前 raw BGRA 主线。
