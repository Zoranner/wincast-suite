...
- 宿主机只有一个需要参与捕获的显示器。
- Windows 10 1809 / Build 17763 仅支持 `auto`/`display` 的唯一显示器捕获兜底；`window` 窗口捕获需要 Windows 10 1903 / Build 18362 或更高版本。
- 待捕获程序能由配置中的 `program`、`args` 和 `work_dir` 启动。
- 目标窗口标题包含稳定可匹配文本，或烟测时明确使用 `capture.mode = "display"` 验证唯一显示器捕获。
...
- `capture.mode` 优先使用 `auto`；Build 17763 宿主机不要使用 `window` 单独验证。
- `capture.startup_timeout_ms` 足够覆盖目标程序启动和窗口出现时间。
